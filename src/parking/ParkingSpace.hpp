...
class ParkingSpace : public Parking {
   ...
   public:
    ParkingSpace(string lot, int number, bool spaceEmpty = true,
                 const int& user = 0) {
        this->lot = lot;
        spaceNumber = number;
    ...
    string getLot() const { return lot; }
    ...
    // displays Parking space number and it its status, also helps
    // Composite uniformity with ParkingLot.hpp
