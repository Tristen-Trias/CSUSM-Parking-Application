...
class ParkingLotDecorator: public ParkingLot  // decorator wraps around ParkingLot
{
  ...
  public:
    ...
    bool reserveSpace(const int id, int spaceNumber, int startTime, int endTime)   // reserves space, with the space number to the base Parkinglot Object
    {
...
class FacultyLotDecorator: public ParkingLotDecorator, public Observer // decorator and observer for the Faculty lot, will add faculty functionality, as well as observers to the lot
{
  public:
   ...
   void display() override//tailors display message for faculty user
    ...
    bool reserveFacultySpot(const int factID, int spaceNumber, int startTime, int endTime) // reserves parking spot for faculty with their ID in parameter and displays its faculty lot
    {
...
class StudentLotDecorator: public ParkingLotDecorator, public Observer // decorator for the Student  lot, will add student  functionality
{
  public:
    ...
    bool reserveStudentSpot(const int studentID, int spaceNumber, int startTime, int endTime) // reserves parking spot for student  with their ID in parameter and displays its student lot
        {
            vector<ParkingSpace>& spaces = baseParkingLot->getSpaces();
            for(int i = 0; i <spaces.size(); i++)
                {
